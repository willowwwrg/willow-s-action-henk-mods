...
public class UIDrawCall : MonoBehaviour
{
	...
	private static string[] ClipRange = new string[4] { "_ClipRange0", "_ClipRange1", "_ClipRange2", "_ClipRange4" };

	private static string[] ClipArgs = new string[4] { "_ClipArgs0", "_ClipArgs1", "_ClipArgs2", "_ClipArgs3" };
	...
	public void UpdateGeometry()
	{
		...
		if (size > 0 && size == uvs.size && size == cols.size && size % 4 == 0)
		{
			...
			if (verts.size < 65000)
			{
				...
				if (!flag2 && panel.renderQueue != UIPanel.RenderQueue.Automatic)
				{
