...
public static class NGUITools
{
	...
	public static void NormalizeWidgetDepths()
	{
		...
		for (int i = 0; i < num; i++)
		{
			...
			int num3 = (uIWidget.depth = num2 + 1);
			num2 = num3;
		}
	}
	...
	{
		Activate(t, compatibilityMode: true);
	}
	...
	public static Vector3[] GetSides(this Camera cam, float depth, Transform relativeTo)
	{
		...
		reference = cam.ViewportToWorldPoint(new Vector3(0f, 0.5f, depth));
		ref Vector3 reference2 = ref mSides[1];
		reference2 = cam.ViewportToWorldPoint(new Vector3(0.5f, 1f, depth));
		ref Vector3 reference3 = ref mSides[2];
		reference3 = cam.ViewportToWorldPoint(new Vector3(1f, 0.5f, depth));
		ref Vector3 reference4 = ref mSides[3];
		reference4 = cam.ViewportToWorldPoint(new Vector3(0.5f, 0f, depth));
		if (relativeTo != null)
		{
			for (int i = 0; i < 4; i++)
			{
				ref Vector3 reference5 = ref mSides[i];
				reference5 = relativeTo.InverseTransformPoint(mSides[i]);
			}
		}
		return mSides;
	}
	...
	{
		ref Vector3 reference = ref mSides[0];
		reference = cam.ViewportToWorldPoint(new Vector3(0f, 0f, depth));
		ref Vector3 reference2 = ref mSides[1];
		reference2 = cam.ViewportToWorldPoint(new Vector3(0f, 1f, depth));
		ref Vector3 reference3 = ref mSides[2];
		reference3 = cam.ViewportToWorldPoint(new Vector3(1f, 1f, depth));
		ref Vector3 reference4 = ref mSides[3];
		reference4 = cam.ViewportToWorldPoint(new Vector3(1f, 0f, depth));
		if (relativeTo != null)
		{
			for (int i = 0; i < 4; i++)
			{
