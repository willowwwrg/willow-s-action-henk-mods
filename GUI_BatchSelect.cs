...
public class GUI_BatchSelect : GUI_Base
{
	...
	private void TransitionCompleted()
	{
		...
		if (!Singleton<LevelBatchManager>.SP.batches[9].IsUnlocked())
		{
			batchButtons[9].SetActive(value: false);
		}
		else
		{
			batchButtons[9].SetActive(value: true);
		}
		...
		Singleton<LevelBatchManager>.SP.lookingAtBatchNum = -1;
		InitMedals();
	}
	...
	private void Update()
	{
		if (!Singleton<PermaGUI>.SP.inbox.activeInHierarchy)
		{
			...
			if (Singleton<InputManager>.SP.CheckAction(InputAction.ExtraLevels) && !Singleton<PermaGUI>.SP.confirmationRequestUp)
			{
				Singleton<GamestateManager>.SP.SetState(typeof(State_BatchSelectExtraLevels));
				AudioController.Play("ButtonForwards");
			}
		}
	}
	...
	public void InitMedals()
	{
		for (int i = 0; i < batchButtons.Count; i++)
		{
			if (!batchButtons[i] || (i == 9 && !batchButtons[i].activeInHierarchy))
			{
				continue;
			}
