...
public class CameraPathAnimator : MonoBehaviour
{
	...
	public Quaternion GetAnimatedOrientation(float percent, bool ignoreNormalisation)
	{
		...
		case orientationModes.target:
		{
			...
			quaternion = Quaternion.LookRotation((!(orientationTarget != null)) ? Vector3.forward : (orientationTarget.transform.position - pathPosition2), targetModeUp);
			break;
		}
		...
		case orientationModes.followTransform:
		{
			...
			quaternion = Quaternion.LookRotation(orientationTarget.transform.position - pathPosition);
			break;
		}
	...
	private void Awake()
	{
		...
		Vector3 eulerAngles = cameraPath.GetPathRotation(_percentage, ignoreNormalisation: false).eulerAngles;
	...
	private void UpdateAnimation()
	{
		...
		else
		{
			...
			{
				if (!animationObjectCamera.isOrthoGraphic)
				{
