...
public class InjectionDetector : ActDetectorBase
{
	...
	public static InjectionDetector Instance
	{
		get
		{
			if (instance == null)
			{
				...
				{
					injectionDetector = new GameObject("Injection Detector").AddComponent<InjectionDetector>();
				}
	...
	private void LoadAndParseAllowedAssemblies()
	{
		...
		for (int i = 0; i < num; i++)
		{
			string[] array = ObscuredString.EncryptDecrypt(binaryReader.ReadString(), "Elina").Split(separator, StringSplitOptions.RemoveEmptyEntries);
