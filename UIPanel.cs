...
public class UIPanel : UIRect
{
			...
			if (mClipping == UIDrawCall.Clipping.None)
			{
				...
				if (transform != null)
				{
					...
					for (int i = 0; i < 4; i++)
					{
						...
						reference5 = transform.InverseTransformPoint(mCorners[i]);
					}
				}
				else
				{
					ref Vector3 reference6 = ref mCorners[0];
					reference6 = new Vector3(num, num2);
					ref Vector3 reference7 = ref mCorners[1];
					reference7 = new Vector3(num, y);
					ref Vector3 reference8 = ref mCorners[2];
					reference8 = new Vector3(x, y);
					ref Vector3 reference9 = ref mCorners[3];
					reference9 = new Vector3(x, num2);
				}
			}
			else
			{
				...
				ref Vector3 reference10 = ref mCorners[0];
				reference10 = new Vector3(num3, num4);
				ref Vector3 reference11 = ref mCorners[1];
				reference11 = new Vector3(num3, y2);
				ref Vector3 reference12 = ref mCorners[2];
				reference12 = new Vector3(x2, y2);
				ref Vector3 reference13 = ref mCorners[3];
				reference13 = new Vector3(x2, num4);
			}
			return mCorners;
		}
	}
	...
	public override Vector3[] worldCorners
	{
		get
		{
			...
			else
			{
				...
				reference5 = transform2.TransformPoint(num3, num4, 0f);
				ref Vector3 reference6 = ref mCorners[1];
				reference6 = transform2.TransformPoint(num3, y2, 0f);
				ref Vector3 reference7 = ref mCorners[2];
				reference7 = transform2.TransformPoint(x2, y2, 0f);
				ref Vector3 reference8 = ref mCorners[3];
				reference8 = transform2.TransformPoint(x2, num4, 0f);
			}
			return mCorners;
		}
	}
	...
	public override Vector3[] GetSides(Transform relativeTo)
	{
		...
		{
			Vector2 viewSize = GetViewSize();
			Vector2 obj = ((mClipping == UIDrawCall.Clipping.None) ? Vector2.zero : ((Vector2)mClipRange + mClipOffset));
			float num = obj.x - 0.5f * viewSize.x;
			float num2 = obj.y - 0.5f * viewSize.y;
			...
			ref Vector3 reference = ref mCorners[0];
			reference = localToWorldMatrix.MultiplyPoint3x4(new Vector3(num, y));
			ref Vector3 reference2 = ref mCorners[1];
			reference2 = localToWorldMatrix.MultiplyPoint3x4(new Vector3(x, num4));
			ref Vector3 reference3 = ref mCorners[2];
			reference3 = localToWorldMatrix.MultiplyPoint3x4(new Vector3(num3, y));
			ref Vector3 reference4 = ref mCorners[3];
			reference4 = localToWorldMatrix.MultiplyPoint3x4(new Vector3(x, num2));
			if (relativeTo != null)
			{
				for (int i = 0; i < 4; i++)
				{
	...
	private void UpdateDrawCalls()
	{
		...
		for (int i = 0; i < drawCalls.size; i++)
		{
			...
			obj.renderQueue = ((renderQueue != RenderQueue.Explicit) ? (startingRenderQueue + i) : startingRenderQueue);
	...
	{
		bool num = mClipping != UIDrawCall.Clipping.None;
		Vector2 result = ((!num) ? new Vector2(Screen.width, Screen.height) : new Vector2(mClipRange.z, mClipRange.w));
		if (!num)
		{
			UIRoot uIRoot = base.root;
			if (uIRoot != null)
