...
public class MB2_MeshBaker : MB2_MeshBakerCommon
{
	...
	private bool _ValidateForUpdateSkinnedMeshBounds()
	{
		...
		if (resultSceneObject.GetComponentInChildren<SkinnedMeshRenderer>() == null)
		{
	...
	public void _update_MB2_MeshCombiner()
	{
		...
		if (resultSceneObject != null)
		{
			Renderer componentInChildren = resultSceneObject.GetComponentInChildren<Renderer>();
			if (renderType == MB_RenderType.skinnedMeshRenderer && !(componentInChildren is SkinnedMeshRenderer))
			{
				meshCombiner.buildSceneMeshObject(resultSceneObject, meshCombiner.GetMesh());
			}
			if (renderType == MB_RenderType.meshRenderer && !(componentInChildren is MeshRenderer))
			{
				meshCombiner.buildSceneMeshObject(resultSceneObject, meshCombiner.GetMesh());
			}
			if (meshCombiner.LOG_LEVEL >= MB2_LogLevel.trace)
			{
				MB2_Log.Trace("Copying settings from MeshBaker to MeshCombiner. Setting target renderer=" + meshCombiner.targetRenderer);
			}
		...
		else
		{
			if (meshCombiner.LOG_LEVEL >= MB2_LogLevel.trace)
			{
				MB2_Log.Trace("Copying settings from MeshBaker to MeshCombiner. Setting target renderer=null");
			}
