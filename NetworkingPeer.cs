...
internal class NetworkingPeer : LoadbalancingPeer, IPhotonPeerListener
{
	...
	public bool ConnectToRegionMaster(CloudRegionCode region)
	{
		...
		if (State == global::PeerState.ConnectedToNameServer)
		{
			return OpAuthenticate(mAppId, mAppVersion, PlayerName, CustomAuthenticationValues, region.ToString());
		}
		string text = NameServerAddress;
		if (!text.Contains(":"))
		{
	...
	public void OnStatusChanged(StatusCode statusCode)
	{
		...
		case StatusCode.Connect:
			...
			{
				didAuthenticate = OpAuthenticate(mAppId, mAppVersion, PlayerName, CustomAuthenticationValues, CloudRegion.ToString());
				if (didAuthenticate)
				{
		...
		case StatusCode.EncryptionFailedToEstablish:
			...
			OpAuthenticate(mAppId, mAppVersion, PlayerName, CustomAuthenticationValues, CloudRegion.ToString());
			break;
	...
	public void OnEvent(EventData photonEvent)
	{
		...
		case 230:
			mGameList = new Dictionary<string, RoomInfo>();
			foreach (DictionaryEntry item in (ExitGames.Client.Photon.Hashtable)photonEvent[222])
			{
			...
			break;
		case 229:
			foreach (DictionaryEntry item2 in (ExitGames.Client.Photon.Hashtable)photonEvent[222])
			{
		...
		case byte.MaxValue:
		{
			...
			if (num == mLocalActor.ID)
			{
				...
				foreach (int num6 in array)
				{
					...
					{
						AddNewPlayer(num6, new PhotonPlayer(isLocal: false, num6, string.Empty));
					}
				}
		...
		case 206:
		{
			...
			for (short num5 = num4; num5 < hashtable.Count; num5++)
			{
				OnSerializeRead(hashtable[num5] as ExitGames.Client.Photon.Hashtable, photonPlayer, networkTime, correctPrefix);
			}
			break;
		}
		case 202:
		...
		case 207:
		{
			int num7 = (int)((ExitGames.Client.Photon.Hashtable)photonEvent[245])[(byte)0];
			if (num7 >= 0)
			{
		...
		case 204:
		{
			int num3 = (int)((ExitGames.Client.Photon.Hashtable)photonEvent[245])[(byte)0];
			GameObject value = null;
			instantiatedObjects.TryGetValue(num3, out value);
			...
			for (int j = 0; j < list.Count; j++)
			{
				...
				else if (parameters.Length - 1 == array2.Length)
				{
					if (CheckTypeMatch(parameters, array2) && parameters[parameters.Length - 1].ParameterType == typeof(PhotonMessageInfo))
					{
						...
						array4[array4.Length - 1] = new PhotonMessageInfo(sender, timestamp, photonView);
	...
	internal GameObject DoInstantiate(ExitGames.Client.Photon.Hashtable evData, PhotonPlayer photonPlayer, GameObject resourceGameObject)
	{
		...
		{
			if (GetMethod(monoBehaviour, PhotonNetworkingMessage.OnPhotonInstantiate.ToString(), out var mi))
			{
	...
	{
		mAppId = appId.Trim();
		mAppVersion = gameVersion.Trim() + "_1.26";
	}
